<x-layouts::auth.login :title="__('Log in')">
    ...
    <form id="element-login-form" method="POST" action="{{ route('elementos.login') }}" data-lookup-url="{{ route('elementos.lookup') }}" class="mt-7">
        @csrf
        ...
        <div class="relative mb-3.5">
            ...
            <span class="mt-0.5 block text-[11px] font-bold text-[#60728b]">Ingresa tu CSP y CUIP para validar tu identidad</span>
        </div>
        ...
        <div class="mb-3.5">
            ...
            <div class="flex">
                ...
                <button id="element-lookup-button" type="button" class="h-8 w-[62px] rounded-e-md border border-l-0 border-[#a46880] bg-white text-[13px] text-[#a46880] hover:bg-[#fcf7f9] focus:z-10 focus:outline-none focus:ring-2 focus:ring-[#d6a5b6]">Buscar</button>
            </div>
            <p id="element-lookup-message" class="mt-1 text-[11px] text-[#b42b43]" role="alert" aria-live="polite"></p>
        </div>
        ...
        <button id="element-submit-button" type="submit" disabled class="h-[34px] w-full rounded bg-[#a46880] text-[13px] font-bold text-white hover:bg-[#925970] focus:outline-none focus:ring-2 focus:ring-[#d6a5b6] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" data-test="login-button">Ingresar al Sistema</button>
